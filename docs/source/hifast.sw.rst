...
-  输出后缀为 ``-sw.hdf5`` 或者 ``-sw_nobld.hdf5``。
...
^^^^^^^^^^^^
   ...
   - ``--nobld``: 是否返回为 未去除基线只去了驻波的数据，后缀为'-sw_nobld.hdf5'。（先去驻波再去基线可能获得更好的基线效果）
...
- 首先对原始数据进行平滑：
   ...
   * 由于时间上的平滑可能扩大应该替代的区域，一旦 ``--s_method_t`` 不为 ``none`` 且 ``restrict_bound True``，
