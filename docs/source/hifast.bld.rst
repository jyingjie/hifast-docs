...
这个过程是把除了信号外的其他成分作为基线拟合并扣除。通常基线比信号的"平滑度"更高，这里用PLS、多项式等方法来拟合基线。基线拟合需要排除信号的区域，这里采用
迭代调节每个数据点的权重来使信号区域的权重为0或者很小来实现。噪音会影响基线拟合的效果，需要通过预处理来降低噪音减小这一影响。

因此这一步的流程为 预处理———迭代拟合基线———后处理(可选，取决于预处理)。输出文件名会包含 ``-bld`` 或者 ``-bld_p``。
...
.. note::
   预处理后的谱线只用于拟合基线，然后再用原始谱线减去基线，因此不影响原始谱线。

预处理包括沿着时间轴和沿着频率方向两种操作。

   -  | 沿时间轴对每个 *channel* (*frequency sample*)操作（开启后可能需加后处理）
      | 包括合并谱线 ``--njoin`` 和平滑 ``--s_method_t`` 两种方法，通常只使用其中一种。``--njoin`` 减少了要拟合的基线数量，能节省之后拟合基线的时间。
...
^^^^^^^^^^^^^^^^^^^
   ...
   -  | ``--method``: 迭代拟合方法
      ...
      | 前半段有 ``PLS`` 或者 ``poly`` （多项式），其中多项式拟合，适用于基线比较简单的情况，例如减去参考点(off-source)谱线后
        残余基线和连续谱的扣除。
      | 后半段有 ``asym1``, ``asym2``, ``asym3`` 三种假设信号位于基线一侧（发射线），三种都有可能造成信号两侧基线过高的情况，
        可配合 ``--exclude_add`` 缓解。
...
后处理
^^^^^^^^^^^^^^^^^^^^^^
只有在预处理时开启了 ``--njoin`` 或者 ``--s_method_t`` 才需要。后处理用低阶多项式再次去基线，也可以把输出文件再次输入 ``hifast.bld`` 来替代。
